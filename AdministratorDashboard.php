<?php
session_start();
if (!isset($_SESSION["authenticated"]) || $_SESSION["type"] != "Administrator") {
...
if(isset($_POST["listUsers"])) {
    $listOfAllUsers = $user->listUsers();
}
$listUsers = isset($_POST["listUsers"]) && $_POST["listUsers"] == "true";


### Search a user functionality
$searchedUserInfo = "";
if (isset($_POST["searchUser"])) {
    $username = $_POST["username"];
    $searchedUserInfoResult = $user->searchUser($username);
    $searchedUserInfo = $searchedUserInfoResult->fetch_assoc();
}


### List all users currently check-in at a parking location functionality
if (isset($_POST["showCheckInUsers"])) {
    $checkInUsers = $user->listCheckInUsers();
}
$showCheckInUsers = isset($_POST["showCheckInUsers"]) && $_POST["showCheckInUsers"] == "true";


### used for checking-in functionality
$checkInMessage = "";
if (isset($_POST["checkIn"])) {
    $username = $_POST["checkInUsername"];  
    $location = $_POST["checkInLocation"]; 
    $intendedDuration = $_POST["intendedDuration"];
    $result = $user->checkIn($username, $location, $intendedDuration, $adminUsername = $_SESSION["username"]);
...
if (isset($_POST["showCheckIns"])) {
    ...
    $currentCheckInsResult = $user->listCurrentCheckIns($username);
    ...
    if (isset($_POST["checkOut"])) {
        ...
        $checkInDetails = null;  # Initialize to null to check later if it was set
        while ($row = $currentCheckInsResult->fetch_assoc()) {
        ...
        if ($checkInDetails) {
            ...
            $checkOutMessage = "Error: Check-in details not found for location $location.";
        }
...
### Insert parking location functionality
if (isset($_POST["insertLocation"])) {
    $location = $_POST["location"];
    $description = $_POST["description"];
    $capacity = $_POST["capacity"];
    $costPH = $_POST["costPH"];
    $lateCheckoutCostPH = $_POST["lateCheckoutCostPH"];
    $availableSpaces = $_POST["availableSpaces"];

    $result = $user->insertLocation($location, $description, $capacity, $costPH, $lateCheckoutCostPH, $availableSpaces);

    if ($result) {
        $insertMessage = "Location inserted successfully!";
    } else {
        $insertMessage = "Failed to insert location.";
    }
}

### Update parking location functionality
if (isset($_POST["updateLocation"])) {
    $location = $_POST["editLocation"];
    $newCapacity = $_POST["newCapacity"];

    $result = $user->editLocation($location, $newCapacity);

    if ($result) {
        $editMessage = "Location updated successfully!";
    } else {
        $editMessage = "Failed to update location.";
    }
}
...
<body>
    ...
    <div id="container">
        ...
        <div class="section">
            ...
            <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                <?php if (!$listUsers): ?>
                    <input type="hidden" name="listUsers" value="true" />
                    <button type="submit" class="button">Show Users</button>
                <?php else: ?>
                    <input type="hidden" name="listUsers" value="false" />
                    <button type="submit" class="button">Hide Users</button>
                <?php endif; ?>
            </form>

            <?php if ($listUsers): ?>
                <ul>
                    <?php while ($row = $listOfAllUsers->fetch_assoc()): ?>
        ...
        <div class="section">
            ...
            <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                ...
                <button type="submit" name="searchUser" class="button">Search</button>
            </form>

            <?php if (isset($_POST["searchUser"])): ?>
                <?php if ($searchedUserInfo): ?>
                    <ul>
                        <li>Username: <?php echo $searchedUserInfo["username"]; ?></li>
                        <li>Name: <?php echo $searchedUserInfo["name"]; ?></li>
                        <li>Phone: <?php echo $searchedUserInfo["phone"]; ?></li>
                        <li>Email: <?php echo $searchedUserInfo["email"]; ?></li>
                    </ul>
                <?php else: ?>
                    <p>No user found with the entered username, please try again.</p>
                <?php endif; ?>
            <?php endif; ?>
        </div>

        <!-------------------------- List All Users Currently Checked In ----------------------------->
        <div class="section">
            <h3>Users Currently Checked In</h3>
            <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                <?php if (!$showCheckInUsers): ?>
                    <input type="hidden" name="showCheckInUsers" value="true" />
                    <button type="submit" class="button">Show Check-In Users</button>
                <?php else: ?>
                    <input type="hidden" name="showCheckInUsers" value="false" />
                    <button type="submit" class="button">Hide Check-In Users</button>
                <?php endif; ?>
            </form>
            ...
            <?php if ($showCheckInUsers): ?>
                <table>
                    ...
                    <?php while ($row = $checkInUsers->fetch_assoc()): ?>
                        <tr>
        ...
        <!-----------------------------Check-in a user for a parking ---------------------------->
        <div class="section">
            <h3>Check-in for Parking</h3>
            <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                <input type="text" name="checkInUsername" placeholder="Enter username you check-in for" style="width:200px;" required />
        ...
        <div class="section">
            <h3>Check-out</h3>
            <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                <input type="text" name="checkOutUsername" placeholder="Enter username you check-out for" style="width:210px"; required />
        ...
        <div class="section">
            <h3>Insert New Parking Location</h3>
            <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                <input type="text" name="location" placeholder="Location" required />
                <input type="text" name="description" placeholder="Description" required />
                <input type="number" name="capacity" placeholder="Capacity" required />
                <input type="number" step="0.01" name="costPH" placeholder="Cost per Hour" required />
                <input type="number" step="0.01" name="lateCheckoutCostPH" placeholder="Late Checkout Cost per Hour" required />
                <input type="number" name="availableSpaces" placeholder="Available Spaces" required />
                <br /><br />
                <button type="submit" name="insertLocation" class="button">Insert Location</button>
            </form>
        ...
        <div class="section">
            ...
            <form method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
                <input type="text" name="editLocation" placeholder="Enter Location to Edit" required />
                <input type="number" name="newCapacity" placeholder="New Capacity" required />
                <button type="submit" name="updateLocation" class="button">Update Capacity</button>
            </form>
